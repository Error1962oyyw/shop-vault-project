...
export function parseLoginError(error: any): LoginErrorResult {
  ...
  if (rawMsg.includes('账户已被暂停') || rawMsg.includes('已被暂停') || rawMsg.includes('Disabled') || rawMsg.includes('禁用')) {
  ...
  } else if (rawMsg.includes('权限不足') || rawMsg.includes('非管理员') || rawMsg.includes('Permission denied') || rawMsg.includes('Not admin')) {
    ...
    result.passwordError = '邮箱或密码错误，请检查后重试'
  } else if (rawMsg.includes('账户已锁定') || rawMsg.includes('已锁定')) {
    result.errorMessage = '账户已锁定，请稍后再试'
